...
const MurottalPlayer: React.FC = ({ data }) => {
  ...
  const handleClickOutside = e => {
    if (
      volumeContentRef.current &&
      !volumeContentRef.current.contains(e.target)
    ) {
      if (volumeToggle) {
    ...
    <PlayerStyled>
      <div className="fixed bottom-0 bg-black z-50 left-0 right-0 border-t border-gray-800">
        <div className="container max-w-screen-lg">
          <div
            className="flex items-center justify-between text-sm -mx-4"
            style={{ height: 60 }}
          >
              ...
              <ul className="flex -mx-2 items-center">
                ...
                <li className="px-2 hidden sm:block">
                  <button className="amplitude-repeat relative" id="repeat">
                    <span className="absolute repeat top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                      1
                    </span>
                ...
                <li className="px-2">
                  <div className="relative">
                    ...
                    <StyledContent
                      ref={volumeContentRef}
                      className={`bg-black border bg-opacity-60 backdrop-filter backdrop-blur rounded-lg border-gray-800 transform left-1/2 -translate-x-1/2 transition-all ${
                        volumeToggle ? "block" : "hidden"
                      }`}
                    >
            ...
            <TimeContainer className="px-2 sm:px-4">
              <div id="time-container" className="flex items-center w-full">
                <span
                  className="current-time pr-2 block flex-none"
                  style={{ width: 48 }}
                >
                ...
                <div id="progress-container" className="relative w-full">
                  ...
                  <input
                    type="range"
                    className="amplitude-song-slider absolute w-full top-1/2 transform -translate-y-1/2"
                  />
                  <progress
                    id="song-played-progress"
                    className="amplitude-song-played-progress absolute w-full top-1/2 transform -translate-y-1/2"
                  >
                  </progress>
                  <progress
                    id="song-buffered-progress"
                    className="amplitude-buffered-progress absolute w-full top-1/2 transform -translate-y-1/2"
                    value="0"
                  >
                  </progress>
                </div>
                ...
                <div>
                  <ul className="flex items-center -mx-1">
                    <li className="px-1  hidden sm:block">
                      <img
                        className="object-cover h-9 w-9 rounded-md album-art"
                    ...
                    <li className="px-1 hidden sm:block">
                      <h3
                        className="line-clamp-1 song-name"
                        data-amplitude-song-info="name"
                      >
                      </h3>
                      <p
                        className="text-blueGray-400 line-clamp-1"
        ...
        <button
          onClick={handleClose}
          className="absolute right-2 z-50 top-1/2 transform focus:outline-none -translate-y-1/2 text-blueGray-500"
        >
          <BsX size={24} />
        </button>
      </div>
