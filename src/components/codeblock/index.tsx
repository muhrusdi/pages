...
const highlightLine = (lineArray, lineProps) => {
  ...
  lineArray.forEach(line => {
    ...
    if (
      !!highlightAddStart &&
      content.replace(/\s/g, "") === "//highlight-add-end"
    ) {
    ...
    if (
      !!highlightRemoveStart &&
      content.replace(/\s/g, "") === "//highlight-remove-end"
    ) {
      ...
      {({ className, style, tokens, getLineProps, getTokenProps }) => (
        <Pre className={`${className} relative`} style={style}>
          ...
          {tokens.map((line, i) => {
            ...
            const { shouldExclude, highlightAddStart, highlightRemoveStart } =
              highlightLine(line, lineProps)
